\documentclass[oneside,1àpt]{article}
...
Soient :
\begin{itemize}
\item A.Kpub la clé publique d'Alexandre.
\item B.Kpriv la clé privée de Benjamin.
\item C.rib la relevé d'identité banquaire de Charlie.
\item D.facture une facture emise pas Dominique.
\item E.paiement l'ordre de paiement venant d'Edouard.
\item S(F.rib, G.Kpub) le rib de Fr\'ederic signé par Gilles.
\item S(H.Kpub) la signature du message courant avec la clé publique de Henri.
\item C(I.rib, J.Kpriv) le rib de Ivan chiffré avec la clé privée de John.
\end{itemize}
...
V.rib+montant+transactionID

Ensuite, Charles paiera Victor avec un message qui aura la forme :
